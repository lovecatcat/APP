...
(function(window, document) {
    var Html5video = function(getDom, options) {
        ...
        this.options = this.extend({
                ...
                //视频截图封面
                time: "00:00",
                //视频总时间以分钟单位，当网络缓慢时，没办法及时加载出来，如已知可以填写，可不填。
                autoplay: false,
                //是否自动播放视频
                isMobile: false,
                //是否开启当处于移动网络时，提示用户是否继续播放视频。
                isFull: false,
                //是否点击播放就全屏显示
                iospay: false,
                //如果是IOS系统是否采用苹果系统自带播放器, 可以在浏览器或微信中，全屏观看视频
                drag: false,
                //禁止拖动,调节,音量和亮度
                isfull: true,
                //是否显示全屏按钮
                prompt: function(video) {},
                //当开启isMobile时,这里可以写提示用户的内容,
    ...
    Html5video.prototype = {
        ...
        event: function() {
            ...
            mui_pay_ico.addEventListener("tap",
                function() {
            ...
            mui_block_range.addEventListener("change",
                function() {
                    var val = this.value;
            ...
            mui_block_range.addEventListener("input",
                function() {
                    var val = this.value;
                    mui_block_range.style.backgroundSize = "" + val + "% 100%";
                });
            ...
            mui_video.addEventListener("loadedmetadata",
                function() {
            ...
            //监测视频是否已开始播放
            mui_video.addEventListener("playing",
                function() {
                    mui_pay_ico.style.display = "none";
                    mui_videoControls.style.display = "none";
                    rprogress.style.display = "block";
            ...
            //监测视频播放进度，让进度条走起来
            mui_video.addEventListener("timeupdate",
                function() {
            ...
            //当播放结束
            mui_video.addEventListener("ended",
                function() {
            ...
            mui_video.addEventListener("error",
                function() {
                    mui.toast("~加载视频失败~");
                });
            ...
            var statbool = false;
            var drags_v = 0;
            v_left.addEventListener("dragstart",
                function(event) {
            ...
            v_left.addEventListener("dragend",
                function(event) {
            ...
            b_right.addEventListener("dragstart",
                function(event) {
                    if (mui.os.plus && !_this.options.drag) {
                        //向下滑动
                        x = event.detail.deltaX;
            ...
            //点击全屏按钮
            mui_quanping.addEventListener("tap",
                function() {
                    isfull("");
                });
            ...
            //后退出全屏播放
            mui_full_back.addEventListener("tap",
                function() {
                    //退出全屏播放
                    if (document.exitFullscreen) {
                        document.exitFullscreen();
