...
proto._initNativeView = function() {
	this.nativeView = new plus.nativeObj.View('__MUI_TAB_NATIVE', {
		'top': this.topHeight + 'px', //这个需要根据顶部导航及顶部选项卡高度自动调整
...
proto._initWebviewContexts = function() {
	...
	for(var len = this.items.length, i = len - 1; i >= 0; i--) {
		...
		var webviewContext = new webviewGroupContext(id, webviewOptions, this, topHeight);
 ...
 */
var webviewGroupContext = function(id, webviewOptions, groupContext, topHeight) {
	this.id = id;
